...
#define FIRST_6_BIT			0xFC00000000000000
#define FIRST_32_BIT		0xFFFFFFFF00000000
#define SECOND_32_BIT		0x00000000FFFFFFFF
#define FIRST_28_BIT		0xFFFFFFF000000000
#define SECOND_28_BIT		0x0000000FFFFFFF00
#define BIT_NO_6			0x20
#define BIT_NO_1			0x1
#define BIT_NO_2345			0x1E
#define GET(x, n)			((x)  &  (1ULL << (SIZE_BITS - (n) - 1)))
#define SET(x, n)			(((x) |= (1ULL << (SIZE_BITS - (n) - 1))))
#define LSHIFT(x, i)		((x) = (((x) << 1) | (((x) & FIRST_BIT) >> 27)))
#define PERMUTE(value, permuted, arr, count) \
		for (int i = 0; i < count; i++) {\
			if (GET(value, arr[i] - 1))\
			{\
				SET(permuted, i);\
			}\
		}
namespace DES {
	...
	uint64_t Feistel(uint64_t Rn, uint64_t Kn);
};